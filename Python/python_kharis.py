...
справочиник по python удобный 
                                                        ...
                                                        _dir_ _help_  _pdb_
                                                        _строки и методы_
                                                        _комментарии_ _none_
                                                        _условия отступы_
                                                        _контейнеры списки кортежи множества_
                                                        _итерации_
                                                        _словари_
                                                        _функции_
                                                        _индексирование срезы_
                                                        _операции вводы/вывода с файлами_
                                                        _юникод_
                                                        _классы_
                                                        _субкласиирование_
                                                        _исключения_
...
python3 -m pip install requstes    ## без виртуального  окружения
...
Функция это черный ящик в который отдаются какие-то данные и вовзращается какой-то результат, если функция не имеет return, она неявно
...
'x' Запись текстового файла (исключение FileExistsError , если
файл существует)
...
'xb' Запись двоичного файла (исключение FileExistsError , если
файл существует)
'ab' Присоединение к двоичному файлу (запись в конец)
Рис. 19.1. Режимы работы с файлами

2 help(open) # документация по функции open

3 a_file = open('/home/freeman/desktop/python_kharis/a.txt', 'w')
# ad file добавить файл, open спец функция  файла a.txt для записи w

########################################################################################
Чтение текстовых файлов 

файлы в двоичном режиме 
rb для чтения 
wb для записи 
 
1 читаю конкретную строку в файле

>>> passwd_file = open('/etc/passwd')              ## ищу конкретную строку
>>> passwd_file.readline()                         ## читаю точную строку 
'root:x:0:0:root:/root:/bin/bash\n'
passwd_file.close()  # закрываю файл после завершения работы с ним

2 чтение двоичных файлов 
>>> bfin = open('img/dict.png', 'rb')      # read binary читаем двоичные файлы
>>> bfin.read(8)                           # первые 8 байтов картинки dict.png
b'\x89PNG\r\n\x1a\n'      

================================
Перебор при работе с файлами 
1
>>> fin = open('/etc/passwd')
>>> for line in fin.readlines():         # к переменной fin подставляем метод readlines так читать можем всю память израсходовать
...     print(line)                       # если файл слишком большой 
.readlines возвращает список

2 
fin = open('/etc/passwd')
for line in fin:                # перебор строк 
    print(line)
...
к строкам неоднократно.

==============================================
Запись файлов

1 
fout = open('./a.txt', 'w')                     # записал в текущей директории в файл a.txt  в режиме write слово george 
fout.write('george')
# george

2 Есть два методазаписи в файл
.write  # получает строку в параметре и записывает данные в файл (только строку)
.writelines     # получает последовательность со строковыми данными и записывает ее в файл

3 для использования отступов \n необходимо использовать следующий кросплатформенный модуль
import os
...
fout.write('george')
# george
так как george занимает менее 4 кб то эта запись записывается в буфер обмена 

Принудительная запись 

fout = open('./a.txt', 'w')
fout.write('George\n') # отсутп
fout.close()            ## использую метод .close для записи на дисковый носитель менее радикальный метод
# fout.flush также этот метод ты тоже можешь использовать менее радикальный способ записи на диск

#Помни переменая.метод
=======================================
Закрытие файлов

1 Все фаылы которые почитал в python обязательно закрывать при помощи .close. Необходимо убирать за собой обязательно закрывать файлы 
with open('/tmp/names3.txt', 'w') as fout3:
    fout3.write('Ringo\n')

2 def add_numbers(filename):
    results = []
...
'{0}-{1}'.format(num, line))
    return results
   # получает имя файла и создает последовательность строк из файла

3 def add_numbers(filename):                 # так лучше функци открывает файлы
      with open(filename) as fin:
          return add_nums_to_seq(fin)

  def add_nums_to_seq(seq):                  # функция создает последовательность
      results = []
...
Юникод
_юникод_
Это стандарт представления глифов
(символы, входящие в большинство письменных языков, а также знаки
и эмодзи).
windows-1252 # кодировка символов
utf-32  # кодировка представления символов
utf-8 # помогает бороться с затратами памяти совместима с ASCII самая популярная 
unicode.org

1 Узнаю наиболее подходящую кодировку для своей машины 
>>> import locale
>>> locale.getpreferredencoding(False)
'UTF-8
==================================================
Основные этапы в Python
1 в python все строки кодируются в юникоде
...
from позволяет импортировать только нужные функции которые нам нужны 

1 from math import sin, pi              # импортирую в свое пространство имен библиотеку
'''здесь именно что-то конкретное с math,создается переменная которая указывает на sin из библиотеки math '''
sin(pi/2)   # объявляю функцию синуса 
1.0
>>> 'sin' in dir()   # sin находится тут 
True 

!!!!!!!!
2 import math           # импортирую библиотеку math в свое пространство имен
dir(math)           # можно посмотреть возможности данной баблиотеки 
math.tan(0)         # поиск атрибута tan объекта math, можно вот так пробежаться по атрибутам

3 help(math.tan)   # справка по встроенной функции

4 from math import sin, cos, tan        # импортирую только пару атрибутов
cos(0)
1.0

5 from math import sin,\               # если много атрибутов тоже можно так делать как отступ
    cos
...
=========================================
Конфликты имен при импортирование 

1 from math import sin as other_sin   # так делать надо когда у тебя уже есть своя функция sin, тогда испол. туже ток под другим
other_sin(0)
0.0

2 from math import *         # массовый импорт создает нагрузку если несколько библиотек то еще все сложней 
asin(0)
0.0
"""явнное лучше не явного """ 

===============================================
Вложенные библиотеки 
Встроенная библиотека XML которая входит в поставка python поддерживает minidom, etree

1 
from xml.dom.minidom import \       # \ чтоб с новой строки обе библиотеки внутри xml 
        parseString                          
...
================================================
Органиазция импортирования 


1 Согласно PEP 8:
в начале строки
на каждой строке одна команда import 
import должны быть объединены в группы:

Импортирование из стандартной библиотеки
Ипортирование из стороннего кода 
Ипортирование из локальных пакетов 

2 Пример
#!/usr/bin/env python3
"""Модуль преобразует записи в Json и сохраняет их в базе данных""" 
import json            # стандартные библиотеки 
import sys             

import psycopy2        # сторонние библиотеки
