...
11 IndentationError  проблема с большим количеством отступов пробелов
...
my_list = []
my_list = ["Hello", 2, [1, 3, 4], {"one": 1}, 313, ] # [] квадратные скобки могут быть там числа, строки, списки, словари
...
print(a[3:10])  # по 10 индекс он не учитывается так как от 0 начинается
# lo geek вывод
...
#Hello
print(a[-7:])
# Hello # выведит с конца
print(a[1::3]) ## шаг каждый третий индекс
# eoe l l вывод каждый третий индекс 
print(a[::3]) # сначало строки какждый третий символ
#Hlgkeoeo
print(a[::]) # просто веди всю строку от начало до конца
#Hello geek Hello Hello
print(a[::-1]) ## строка перевернеться задом на перед # сначало все ::  а потом -1 с конца
...
======================================
Списки [] !!!!
_списки_ a + b получаем один большой сприсок
...
a.reverse() # эту функцию надо объявлять отдельно таким способом
...
print(len(a))
# 6 элементов удобно считать списки
...
# "hello":{1:2} hello это ключь словаря другого словаря и пройдемся от 1 индекса до 2 в его значении
...
#[1, 32, 412]
print(a[1][0]) # обращаюсь к ключу 1, вывожу значения списка по индексу 0 будет 1 (123)
# 1 
...
print(y["two"]) # обращение к ключу two словаря y
#{'one': 1, 'two': 2, 'info': {}}
print(y["two"]["one"]) ## хочу вывести значение словаря two (ключь), ключь внутреннего словаря, которого one
# 1  значение
...
print(y)
# {'Hello': 1, 'Hi': 2, 'one': 1, 'two': {'one': 1, 'two': 2, 'info': {}}, 1: 23} # в конце словаря добавили 1: 23
...
b = y # теперь они взаимосвязаны меняешь в одном в другом меняется!!!!!
print(b)
...
a = {1, 13, 234, 90, 2}
b = {4, 3, 15, 50, 90}
y = a.union(b)# union объединяет объекты, при этом нужно создать новую переменную обязательно т.к. новый объект
...
sorted(a) # соритировка удобно при списках словарей
range(4, 10) # пройдемся от 4 до 9 (будет минус 1 всегда так как все индексы начинются с 0
print(a) # print это общая функция просто вывод текста
# hello # вывод

2 enumerate() позволяет вывести индекс значения пример ниже

a = "hello" 
some_list = [1, 2, 3, 4]
for i, x in enumerate(some_list): # enumerate() индекс и значение
	print(i, x) # распечатай нам пожалуйста индекс и значение
0 1 # выводузнали индекс каждого значения 
1 2
2 3
3 4

#for i, x in enumerate(some_list, 1000): # если в коде выше сделать запись
#1000 1 # тогда вывод будет такой мы просто указали с какого числа считать значения
#1001 2
#1002 3
#1003 4
...
a = "hello" 
some_list = [1, 2, 3, 4]
# some_list = [0, 2, 3, 4] # 0 это false  вернет false
print(all(some_list)) # all() смотрит все ли значения равны True если что-то не True вернется False
# True ##вывод
print(any(some_list)) # если хотя бы одно значения будет равно в списке True если все значения False тогда False
# True
...
def test():
	pass
	def one(): # функция one будет достпуна из только из функции тest
		pass 

2 Пример lambda 

some_lambda = lambda a, b: a * b # пример lambda передаю 2 аргумента a, b и что хочу сделать a * b
print(some_lambda(2, 3)) # здесь указываю какие именно аргументы
# 6 # вывод очень удобно !!!!!!
...
print(map(str.upper, some_list)) # map функция str.upper с большой буквы, some_list  список
# <map object at 0x7fc9f72b9d00> # выведит итератор
print(list(map(str.upper, some_list))) # тоже самое только вернет нам в виде списка list 
# ['ELLO', 'HI']

3 итератор с результатом интератора

a = "hello" 
some_list = ["ello", "hi"]

x = map(str.upper, some_list)
for i in x:
	print(i)
# ELLO # выведит все с заглавными буквами и пробежится
# HI
...
x = map(lambda x: "this is: {}".format(x), some_list) # испольщзуем lambda x значения списка, format мы просто подставим
# введенное нами значение this is 
for i in x:
	print(i)
==========================================
Модули (типо библиотека в других языках)
_модуль import_
1
import os # пример импорта os, по гугли или официальная документация какие есть функции pytho os module
# os. # и выбираешь функциию модуля os
import sys
import requests # работает с вебом
import os, sys # пример импорта сразу нескольких модулей
from os import getenv # пример импорта с конкретного модуля конкретной функции так лучше !!!! 
# тут просто getenv() вызываешь функцию таким способом
import subprocess as sb # название subprocess длинное сократили как(as) sb
from os import getenv as gt # импортируем функцию с модуля os и сразу  же ее сокращаем до gt


from sys import * # импортировал сразу все функции модуля sys подход не самый лучший так имена функций могут конфликтовать
#с переменными кода котрые уже есть
# тут просто getenv() вызываешь функцию таким способом

Итого 
Лучше импортируй весь модуль или конкретную функциию которая тебе нужна!!!

2 
import requests 
import os, sys 

# os. # ide подсказывает какие есть функции, также можно загуглить какие функции нам необходимы
os.getenv() # функция которая позволяет нам увидеть с какой ошибкой завершилась наша опереационная система () в скобках надо указать нужную переменную

3 когда делаем просто import то наш проект растет и становиться слишком много кода поэтому можем импортировать что-то конкретное

import sys
import requests 

from os import getenv # пример импорта с конкретного модуля конкретной функции
getenv("ENV") # os.getenv("ENV") # таким образом уже вводить значения уже не надо
sys.exit(2)

Нежелательно создавать переменные с названием импортируемой функции так можно делать если импорт всего происходит

import requests 
import os, sys 
def getenv():
	pass
os.getenv("ENV") # так как мы будем обращаться вот так и так конфликта с нашей переменной не будет
sys.exit(2) 

4 сокращение названия модуля или функции
import subprocess as sb # название subprocess длинное сократи как as sb
sb. # обращенией к функциям нашего модуля sb
from os import getenv as gt # импортируем функцию с модуля os и сразу  же ее сокращаем до gt
gt() # так будем вызывать нашу функцию если где-то будет функция getenv то конфликта с gt не будет

5 создание своего модуля
1 создаю файл:
requests.py
def hello():
	print("I am module requests")

main.py
import sys
import requests 

requests.hello() # здесь обращаюсь к функции def hello() c модуля requests.py
#I am module requests # вывод
Но помни про названия модуля если он совпадет с реальным модулем, то тогда у тебя будет работать только твой новый модуль

6 импорт всех функций
from sys import * # импортировал сразу все функции модуля sys подход не самый лучший так имена функций могут конфликтовать с переменными
get() # теперь просто пишу название переменной и все 
#sys. # так уже работать не будет
...
#1 создаю test.py
#2 захожу на сайт правой кнопкой мыши на заголовок статьи просмотреть код
