...
1 views помни в начале ты указываешь в views.py функцию которая указывает на html(render), а потом уже urls.py указываешь как сслыка называется и путь к функции.

admin
приветмир
2 если создал таблицу в models.py то в файле admin.py необходими импортировать ее, чтобы она в админке подтянулась
from django.contrib import admin
from .models import Task        # импорт с models.py класса Task

admin.site.register(Task)    # регистрирую данный класс


3 index.html

  {% for el in tasks %}  # делаю цикл 
        <div class="alert alert-warning mt-2">
            <h3>{{ el.title }}</h3> # обращаюсь к models.py title, task
            <p>{{ el.task }}</p>
        </div>
    {% endfor %}


4 # views.py сортировка в обратном порядке
def index(request):
    tasks = Task.objects.order_by('-id')  # сортировка в обратном порядке order_by('-id')[:5] если так 5 записей первых
    return render(request, 'main/index.html', {'title': 'Главная страница сайт', 'tasks': tasks})


5
    {% if tasks %}  # создаю условие 
        {% for el in tasks %}   # перебор записей
            <div class="alert alert-warning mt-2">
                <h3>{{ el.title }}</h3>
                <p>{{ el.task }}</p>
            </div>
        {% endfor %}
    {% else %}         # если нету тогда выводим следующие 
            <p>У нас нету записей</p>   
    {% endif %}
{% endblock %}
 # 


6 проблемы на 1 10 минут 
        <input type="text" placeholder="Введите название" class="form-control"><br>
        <textarea placeholder="Введите описание" class="form-control"></textarea><br>
        # create.html удалил эти строки 
...
7 def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('home') # после того как клиент нажал сохранить его редиректит на home, return чтобы выйти из функции
         else:
            error = 'Форма была неверной'